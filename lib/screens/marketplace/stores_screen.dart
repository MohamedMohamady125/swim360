...
<body class="text-gray-800">
    ...
    <script>
        // --- MOCK DATA ---
        let state = {
            currentView: 'stores-list', // 'stores-list', 'store-profile', 'store-products', 'product-detail'
            selectedStoreId: null,
            selectedCategory: null,
            selectedProductName: null, // New state for product details
            cart: [],
            // Product variants state: holds the user's selected size/color while on the detail page
            productVariants: {
                size: null,
                color: null
            },
            ...
            // Updated mock products with multiple photos and detailed info
            storeProducts: { 
                'store1': {
                    'Suits': [
        ...
        function renderApp() {
            ...
            switch (state.currentView) {
                ...
                case 'product-detail':
                    ...
                    const productStore = state.stores.find(s => s.id === state.selectedStoreId);
        ...
        function renderStoreProfileView() {
            ...
            const mostSoldItemsHtml = store.mostSold.map(productName => {
                ...
                return `
                    <button onclick="navigate('product-detail', '${store.id}', '${details.category}', '${details.product.name}')" 
                            class="bg-blue-50 hover:bg-blue-100 px-3 py-1 rounded-full text-sm font-medium text-blue-800 transition duration-150 shadow-sm whitespace-nowrap">
                        ${productName}
                    </button>
                `;
            }).join('');
        ...
        function renderProductDetailView() {
            ...
            const isSizeSelected = (product.availableSizes?.length > 0 && state.productVariants.size !== null) || (product.availableSizes?.length === 0);
            const isColorSelected = (product.availableColors?.length > 0 && state.productVariants.color !== null) || (product.availableColors?.length === 0);

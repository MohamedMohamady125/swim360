...
<body class="text-gray-800">
    ...
    <!-- Store Cart FAB (Hidden in dashboard views, only for store views) -->
    <button id="cart-fab" title="Shopping Cart" class="fixed bottom-6 right-6 w-16 h-16 bg-blue-600 text-white rounded-full shadow-lg flex items-center justify-center transform hover:scale-110 transition-transform z-40 hidden">
    ...
    <script>
        ...
        let state = {
            currentView: 'home',
            // Defaulting the role for initial load to Swimmer
            mockRole: 'swimmer', 
            selectedStoreId: null,
            cart: [],
            // Other state properties related to products...
        };
        ...
        function renderApp() {
            let content = '';
            let headerTitle = 'Swim 360 Dashboard';

            // Only rendering the Home View for this fix to ensure stability
            content = renderHomeView();
            
            // Re-select DOM elements inside renderApp just to be super safe
            appContainer = document.getElementById('app-container');
            cartCountDisplay = document.getElementById('cart-fab').querySelector('.cart-badge');
            
            // Check if element query succeeded before trying to set innerHTML
            if (appContainer) {
                appContainer.innerHTML = renderFullPageLayout(content, headerTitle, null);
            } else {
                 console.error("Critical Error: appContainer not found.");
            }
        }
        ...
        function renderHomeView() {
            const mockRole = state.mockRole; 
            const userName = 'John Doe';
            const welcomeMessage = `Hello, ${userName}!`;

            if (mockRole === 'swimmer' || mockRole === 'parent') {
                return renderSwimmerDashboard(welcomeMessage);
            } else if (mockRole === 'academy') {
                return renderAcademyDashboard(welcomeMessage);
            }
            // Fallback for other roles or missing UI is generic
            return `
                <div class="p-4 pt-12 text-center text-gray-500">
                    Dashboard for ${mockRole.toUpperCase()} not fully implemented yet.
                </div>
            `;
        }
        
        function renderSwimmerDashboard(welcomeMessage) {
            // IMPLEMENTING FINAL REQUEST: Notifications, Book Your Session banner, 4 actions (Orders, Bookings, Programs, Calendar)
            return `
                <div class="p-4 md:p-8">
                    ...
                    <!-- Top Action Banner: BOOK YOUR SESSION (Requested Banner) -->
                    <div class="bg-blue-600 rounded-xl shadow-lg p-6 flex justify-between items-center text-white mb-6 cursor-pointer transform hover:scale-[1.01] transition duration-200" onclick="showSnackbar('Navigating to Book Your Session...', false)">
                    ...
                    <div class="grid grid-cols-2 gap-3 mb-8">
                        ...
                        <!-- 4. Calendar -->
                         <button class="action-card p-4 text-center" onclick="showSnackbar('Navigating to Calendar...', false)">
                            <svg class="w-8 h-8 text-blue-600 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                    ...
                    <div class="flex overflow-x-auto space-x-4 pb-4 poster-scroll">
                        ...
                        <div class="w-64 flex-shrink-0 rounded-xl overflow-hidden shadow-lg border border-gray-200 cursor-pointer bg-red-100" onclick="showSnackbar('Viewing Featured Event: Stroke Clinic', false)">
                            <div class="p-3">
        ...
        function renderAcademyDashboard(welcomeMessage) {
             return `
                <div class="p-4 md:p-8">
                    ...
                    <!-- Primary Action Banner: BOOK YOUR SESSION -->
             ...
             return `
                <div class="p-4 md:p-8">
                    ...
                    <!-- Core Action Cards (2x1 Grid) --><div class="grid grid-cols-2 gap-3 mb-8">
                        ...
                        <!-- Book Your Session --><button class="action-card p-4 text-center" onclick="showSnackbar('Navigating to Client Roster...', false)">
                            ...
                            <span class="text-sm font-semibold">Book Your Session</span>
                        </button>
        ...
        window.onload = () => {
            ...
            // Set the role for the requested dashboard preview
            localStorage.setItem('selectedRole', 'swimmer'); 

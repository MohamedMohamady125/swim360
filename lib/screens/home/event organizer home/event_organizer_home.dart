...
<body class="text-gray-800">
    ...
    <script>
        ...
        function renderHomeView() {
            ...
            const mockRole = localStorage.getItem('selectedRole') || 'event-organizer'; // Testing Event Organizer
        ...
        function renderOrganizerDashboard(welcomeMessage) {
             return `
                <div class="p-4 md:p-8">
                    <h1 class="text-3xl font-extrabold text-gray-800 mb-2">${welcomeMessage}</h1>
                    <p class="text-gray-500 mb-6">Event Organizer Dashboard.</p>
                    ...
                    <div class="bg-blue-600 rounded-xl shadow-lg p-6 flex justify-between items-center text-white mb-6 cursor-pointer transform hover:scale-[1.01] transition duration-200" onclick="showSnackbar('Navigating to Create Event Form...', false)">
                        <span class="text-xl font-extrabold">CREATE EVENT <span class="text-2xl ml-2">&gt;</span></span>
                    ...
                    <div class="grid grid-cols-2 gap-3 mb-8">
                        ...
                        <!-- My Events -->
                        <button class="action-card p-4 text-center" onclick="showSnackbar('Navigating to My Events List...', false)">
                            <svg class="w-8 h-8 text-blue-600 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                            <span class="text-sm font-semibold">My Events</span>
                        </button>
                        
                        <!-- My Attendees -->
                        <button class="action-card p-4 text-center" onclick="showSnackbar('Navigating to My Attendees List...', false)">
                            ...
                            <span class="text-sm font-semibold">My Attendees</span>
                        </button>
                        
                    </div>

                    <!-- Dynamic Poster/Offers Section (Scrolling Banner) -->
                    <h2 class="text-xl font-bold mb-3">Promotional Tools</h2>
                    <div class="flex overflow-x-auto space-x-4 pb-4 poster-scroll">
        ...
        function addToCart(productName, storeName) {
            const store = state.stores.find(s => s.id === storeName);
            const categoryProducts = state.storeProducts[store.id]?.[state.selectedCategory] || [];
            const product = categoryProducts.find(p => p.name === productName);
